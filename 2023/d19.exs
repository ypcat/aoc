...
defmodule D19 do
  def parse(input) do
    ...
    |> String.split("\n")
    |> Enum.into(%{}, fn s ->
      [name, rules] = s |> String.split(~r/[{}]/, trim: true)
      rules = rules
      |> String.split(",")
      |> Enum.map(fn r ->
        case String.split(r, ~r/\b/, trim: true) do
          [goto] -> fn _ -> goto end
          [k, ">", x, ":", goto] -> fn p -> p[k] > String.to_integer(x) && goto end
          [k, "<", x, ":", goto] -> fn p -> p[k] < String.to_integer(x) && goto end
        end
      end)
      rule = fn p -> Enum.reduce_while(rules, false, fn r, acc ->
          acc && {:halt, acc} || {:cont, r.(p)}
        end)
      end
      {name, rule}
    end)
    ...
    |> Enum.map(fn s -> s
      ...
      |> Enum.into(%{}, &String.split(&1, "=") |> then(fn [k, x] -> {k, String.to_integer(x)} end))
    end)
    {workflows, parts}
  end
  def go(_workflows, _part, name) when name in ~w(A R), do: name
  def go(workflows, part, name), do: go(workflows, part, workflows[name].(part))
  def part1({workflows, parts}) do
    for p <- parts,
        "A" <- [go(workflows, p, "in")],
        v <- Map.values(p),
        reduce: 0, do: (acc -> acc + v)
  end
  def part2(input) do
    workflows = input
    |> String.split("\n\n", trim: true)
    ...
    |> Enum.into(%{}, fn s ->
      ...
      |> Enum.map(fn r ->
        case String.split(r, ~r/\b/, trim: true) do
          [goto] -> {"x", &Kernel.>/2, 0, goto}
          [k, ">", x, ":", goto] -> {k, &Kernel.>/2, String.to_integer(x), goto}
          [k, "<", x, ":", goto] -> {k, &Kernel.</2, String.to_integer(x), goto}
        end
      end)
      {name, rules}
    end)
    go2(workflows, [{"in", Map.from_keys(~w"x m a s", {1, 4000})}], 0)
  end
  def go2(_workflows, _parts=[], sum), do: sum
  def go2(workflows, [{name, part} | rest], sum) do
    {parts2, sum2} =
    for {name2, part2} <- apply_rules(workflows[name], part, []),
        name2 != "R", reduce: {rest, sum}
    do
      {acc_parts, acc_sum} ->
        case name2 do
          "A" -> {acc_parts, acc_sum + eval(part2)}
          _ -> {[{name2, part2} | acc_parts], acc_sum}
        end
    end
    go2(workflows, parts2, sum2)
  end
  def apply_rules([]=_rules, _part, results), do: results
  def apply_rules(_rules, nil=_part, results), do: results
  def apply_rules([rule | rest], part, results) do
    {part_true, part_false} = split_part(part, rule)
    apply_rules(rest, part_false, List.wrap(part_true) ++ results)
  end
  def split_part(part, {k, op, x, goto}) do
    {first, last} = part[k]
    case {op.(first, x), op.(last, x)} do
      {true, true} -> {{goto, part}, nil}
      {false, false} -> {nil, part}
      {true, false} -> {{goto, %{part | k => {first, x - 1}}}, %{part | k => {x, last}}}
      {false, true} -> {{goto, %{part | k => {x + 1, last}}}, %{part | k => {first, x}}}
    end
  end
  def eval(part) do
    for {first, last} <- Map.values(part), reduce: 1 do
