...
|> Enum.with_index(fn r, i ->
  to_charlist(r)
  |> Enum.with_index(fn c, j -> {{i, j}, c} end)
end)
|> List.flatten()
|> Map.new()
|> then(fn m ->
  ...
  p0 = Enum.find_value(m, fn {p, c} -> if c == ?S, do: p end)
  Enum.reduce_while(m, [p0, p0], fn _, [p, last|_]=visited ->
    case adj.(p) |> Enum.find_value(&p in adj.(&1) && &1 != last && &1) do
      ^p0 -> {:halt, visited}
      p1 -> {:cont, [p1 | visited]}
    end
  end)
  |> then(&div(Enum.count(&1) - 1, 2))
end)
...
|> then(fn m ->
  ...
  p0 = Enum.find_value(m, fn {p, c} -> if c == ?S, do: p end)
  loop = Enum.reduce_while(m, [p0, p0], fn _, [p, last|_]=visited ->
    case adj.(p) |> Enum.find_value(&p in adj.(&1) && &1 != last && &1) do
      ^p0 -> {:halt, visited}
      p1 -> {:cont, [p1 | visited]}
    end
  ...
  s = adj.(p0)
  |> Enum.filter(&p0 in adj.(&1))
  |> Enum.reduce(fn {i1, j1}, {i2, j2} -> {i1 - i2, j1 - j2} end)
  |> case do
    {_, 0} -> ?|
    {0, _} -> ?-
    {1, 1} -> ?L
  ...
  m = %{m | p0 => s}
  {top, left} = Enum.reduce(loop, fn {i, j}, {im, jm} -> {min(i, im), min(j, jm)} end)
  {bottom, right} = Enum.reduce(loop, fn {i, j}, {im, jm} -> {max(i, im), max(j, jm)} end)
  Enum.reduce(top..bottom, MapSet.new(), fn i, acc ->
    Enum.reduce(left..right, {false, nil, acc}, fn j, {side, corner, set} ->
      case {{i, j} in loop, m[{i, j}], side, corner} do
        {true, ?|, _, _} -> {!side, nil, set}
        {true, ?L, _, _} -> {side, ?L, set}
        {true, ?F, _, _} -> {side, ?F, set}
        {true, ?7, _, ?L} -> {!side, nil, set}
        {true, ?J, _, ?F} -> {!side, nil, set}
        {false, _, true, _} -> {side, corner, MapSet.put(set, {i, j})}
        _ -> {side, corner, set}
      end
    end)
    |> elem(2)
    #|> tap(&Enum.map(left..right, fn j -> {i, j} in &1 && ?I || {i, j} in loop && m[{i, j}] || ?. end)|>IO.inspect())
  end)
end)
|> Enum.count()
